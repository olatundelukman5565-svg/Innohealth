#!/bin/bash
# Double-click this file on Linux to start the ThermalMesh web platform
# (backend API + web app) and open it in your browser.
cd "$(dirname "$0")" || exit 1
ROOT_DIR="$(pwd)"

if ! command -v python3 >/dev/null 2>&1; then
  echo "Python was not found. Install Python 3.11+ and try again."
  read -p "Press Enter to close..."
  exit 1
fi
if ! command -v node >/dev/null 2>&1; then
  echo "Node.js was not found. Install Node.js 18+ and try again."
  read -p "Press Enter to close..."
  exit 1
fi

echo "============================================"
echo " Setting up the backend (first run only)..."
echo "============================================"
cd "$ROOT_DIR/web/backend" || exit 1
if [ ! -d ".venv" ]; then
  python3 -m venv .venv
  source .venv/bin/activate
  python3 -m pip install --quiet -r requirements.txt
  if [ $? -ne 0 ]; then
    echo ""
    echo "Backend dependency installation failed -- see the message above."
    read -p "Press Enter to close..."
    exit 1
  fi
fi
[ -f .env ] || cp .env.example .env

echo "============================================"
echo " Setting up the web app (first run only)..."
echo "============================================"
cd "$ROOT_DIR/web/frontend" || exit 1
if [ ! -d "node_modules" ]; then
  npm install
  if [ $? -ne 0 ]; then
    echo ""
    echo "Frontend dependency installation failed -- see the message above."
    read -p "Press Enter to close..."
    exit 1
  fi
fi
[ -f .env.local ] || cp .env.example .env.local

launch_in_terminal() {
  local title="$1"
  local dir="$2"
  local cmd="$3"
  if command -v gnome-terminal >/dev/null 2>&1; then
    gnome-terminal --title="$title" -- bash -c "cd '$dir' && $cmd; exec bash"
  elif command -v konsole >/dev/null 2>&1; then
    konsole --new-tab -p tabtitle="$title" -e bash -c "cd '$dir' && $cmd; exec bash" &
  elif command -v xterm >/dev/null 2>&1; then
    xterm -T "$title" -e bash -c "cd '$dir' && $cmd; exec bash" &
  else
    return 1
  fi
  return 0
}

echo "Starting the backend and web app..."
if launch_in_terminal "ThermalMesh Backend" "$ROOT_DIR/web/backend" "source .venv/bin/activate && uvicorn app.main:app --host 127.0.0.1 --port 8000"; then
  BACKEND_IN_TERMINAL=1
else
  ( cd "$ROOT_DIR/web/backend" && source .venv/bin/activate && nohup uvicorn app.main:app --host 127.0.0.1 --port 8000 > "$ROOT_DIR/backend.log" 2>&1 & )
  BACKEND_IN_TERMINAL=0
fi

if launch_in_terminal "ThermalMesh Web App" "$ROOT_DIR/web/frontend" "npm run dev"; then
  FRONTEND_IN_TERMINAL=1
else
  ( cd "$ROOT_DIR/web/frontend" && nohup npm run dev > "$ROOT_DIR/frontend.log" 2>&1 & )
  FRONTEND_IN_TERMINAL=0
fi

echo ""
echo "Waiting for the servers to start (the backend seeds demo data the very"
echo "first time, which can take about a minute)..."
sleep 10

if command -v xdg-open >/dev/null 2>&1; then
  xdg-open http://localhost:3000 >/dev/null 2>&1
else
  echo "Open http://localhost:3000 in your browser."
fi

echo ""
if [ "$BACKEND_IN_TERMINAL" = "0" ] || [ "$FRONTEND_IN_TERMINAL" = "0" ]; then
  echo "No supported terminal emulator was found, so the server(s) are running"
  echo "in the background. Logs: $ROOT_DIR/backend.log and $ROOT_DIR/frontend.log"
  echo "To stop them later: pkill -f 'uvicorn app.main:app' ; pkill -f 'next dev'"
else
  echo "Two terminal windows just opened for the backend and web app."
  echo "Leave both open while you use the platform -- closing them stops the app."
fi
echo "If the page in your browser shows an error, wait a bit and refresh --"
echo "the backend may still be finishing its first-time setup."
echo ""
echo "Demo login: demo@innohealth.com / demo1234  (admin: admin@innohealth.com / admin123)"
echo ""
read -p "Press Enter to close this window (the app keeps running)..."
