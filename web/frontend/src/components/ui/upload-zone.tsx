  ...
  return (
    <div className="space-y-3">
      <div
        ...
        className={cn(
          "group flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-surface/60 px-6 py-10 text-center transition-colors",
          isDragging && "border-brand bg-brand/5",
          disabled && "cursor-not-allowed opacity-50"
        )}
      >
        <UploadCloud className={cn("h-8 w-8 text-muted transition-colors", isDragging && "text-brand")} />
      ...
      {files.length > 0 && (
        <ul className="space-y-2">
          {files.map((file) => (
            <li key={file.name} className="flex items-center gap-3 rounded-lg border border-border bg-surface/60 px-3 py-2 text-sm">
              {file.status === "error" ? (
              ...
              {onRemove && (
                <button
                  ...
                  className="rounded-md p-1 text-muted hover:bg-white/[0.06] hover:text-danger"
